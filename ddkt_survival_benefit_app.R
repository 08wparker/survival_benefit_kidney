...
survival_benefit <-  function(t_transplant, 
    ...
    if (output == "rmst"){
        ...
        return(
            ...
            paste0("The Lives Saved from Transplant within 5 years (LiST-5) for this donor-recipient pair is ", round(filter(rmst_df, time == 5*365)$surv_benefit, digits = 2), ".")
        )
    }
    if (output == "table"){
        benefit_df <- df %>%
        ...
        return(
            benefit_df %>%
                mutate(S_tx = S_tx,
                       S_wait = S_wait,
                       surv_benefit = S_tx-S_wait,
                       surv_benefit = paste0(round(surv_benefit, 2)),
                ...
                select("Years from transplant" = years_from_transplant,
                       ...
                       "Lives Saved from Transplant (LiST)" = surv_benefit) %>%
                mutate(across(c(2,3), percent))
        )
...
ui <- fluidPage(
    ...
    # Application title
    titlePanel("Saving More Lives with Deceased Donor Kidney Transplantation"),

    # Sidebar for inputs into survival benefit function
    sidebarLayout(
        sidebarPanel(
            h4("Recipient characteristics at the time of transplantation"),
            sliderInput("Age",
                        "Age:",
                        min = 18,
                        max = 80,
            ...
            sliderInput("d_time", "Dialysis Time (years)", 3.5, min = 0, max = 10, step = 0.1),
        ...
        mainPanel(
           ...
           textOutput("rmstText"),
           tags$head(tags$style("#rmstText{color: red;
                                 font-size: 18px;
                                 font-style: bold;
                                 }"
           )),
           ...
           tagList("Survival with and without transplant estimated from all adult candidates listed for deceased donor kidney-alone transplanation, 2005-2010. Model c-stastistic 0.69, indicating good discrimination. Full report and statistical appendix available at:", a("https://github.com/08wparker/survival_benefit_kidney", href="https://github.com/08wparker/survival_benefit_kidney"))
        )
    )
)
...
server <- function(input, output, session) {
    ...
    output$survPlot <- renderPlot({
        ...
        if (input$ctr_id == "mean center"){
            survival_benefit(input$t_transplant, 
                                   "mean center",
