...
Overall, there was a strong positive correlation between the risk of death without a transplant (medical urgency) and survival benefit ($\rho=$ `r round(cor(to_plot$benefit, -to_plot$S_wait, use = "complete.obs"), 2)`) (**Figure 2**). The maximum benefit from deceased donor kidney transplantation was a `r round(100*max_benefit)`% absolute improvement in five-year survival when five-year mortality without a transplant was `r max_waitlist_risk`%, corresponding to ten lives saved with every `r 10*round(1/max_benefit,1)` transplants (within the first five years).
...
dtime_list <- seq(0, 10, 1)

l_dtime <- list(trans_time <- rep(median_prev_transplant,2*length(dtime_list)),
...
```
Five-year survival benefit of deceased donor kidney transplantation by dialysis time and history of diabetes for the median age recipient (`r median_age` years old) transplanted after `r median_t_transplant` with a `r median_kdpi`% donor kidney with `r round(median_ischemic_time)` hours of ischemic time.
...
dial_dm_plot <- survival_benefit(median_t_transplant, "mean center", median_age, med_dialysis_time_no_premept, 1, 0,
                 median_ischemic_time,
                 median_kdpi)


prempt_plot <- survival_benefit(median_t_transplant, "mean center", median_age, 0, 0, 0,
                 median_ischemic_time,
                 median_kdpi)
...
ggsave("dial_dm_vs_preemptive.pdf", plot = prememptive_vs_plot)
```
