---
title: "Calculating the Survival Benefit of Deceased Donor Kidney Transplantation"
...
3.  Develop an accessible end-application for shared decision making between transplant programs and patients when considering deceased donor kidney offers that presents more information than KDPI alone.
...
tbl_summary(recips %>%
              mutate("Post-transplant follow-up time" = t_2 - t_1)%>% 
              select("Age" = age,
                     "Time on waitlist pre-transplant" = t_1,
                     "Dialysis time (years)" = dialysis_time_years,
                     "History of diabetes" = diabetes,
                     "History of previous organ transplant" = CAN_PREV_TX,
                     "Raw Estimated Post-transplant Survival (EPTS) score" = raw_epts,
...
We estimated our model with maximum likelihood estimation using the `coxme` package in `R` [@therneau2018_coxme]. The results are below with the coefficients represented on the log hazard scale. Transplantation is associated with a large negative hazard (better survival). While many of the interaction terms are very small in magnitude and not statistically significant, we chose to retain for all EPTS candidate covariates:donor interactions to control for selection behaviors by transplant physicians.

## Table 3: Mixed-effects cox proportional hazards model results

```{r summarise_main_model}
summary(full_me_model)
```

This fit is from all N = `r to_model %>% filter(tx ==0) %>% nrow() %>% comma()` adult candidates listed for kidney alone deceased donor transplant from 2005 to 2010.

### Transplant center effects (log hazard scale)
...
ranef(full_me_model)$center %>% 
  ...
  head() %>%
  kable(col.names = c("Center", "Waitlist intercept", "Transplant Effect"), digits = 2)
```
...
The hazard ratio of transplantation with KDPI `r median_kdpi`% Kidney for this median patient is `r round(HR_transplant_median, digits = 2)`. The absolute benefit of DDKT increases over time since transplantation, growing each year. Calculating the lives "saved" from DDKT requires a time point to assess survival. For example, the +30% survival benefit at 5-year means that for every 10 "median" DDKT performed will lead to 3 more patients alive at 5-years. Alternatively, the number needed to transplant to save one life (within 5 years) is 3.3.
...
Holding the characteristics of the "median" recipient constant (`r median_age` years old, listed and transplanted at the average center, on dialysis for `r median_d_time` years, `r median_t_transplant` days of waiting), we now calculate the survival benefit of DDKT ranging the KDPI from 1% to 99% (with `r round(median_ischemic_time)` hours of ischemic time)
...
# Exploring interactions between candidate and donor characteristics
...
$$\hat{S}(t|accept \, W_j)_{ijk} = exp(-\int_{t_{a}}^t \hat{h}_{ijk}(transplant \, W_{j})) dt$$
...
$$\hat{S}(t|wait \,for \, W_m)_{ijk} = exp(-(\int_{t_{a}}^{t_b} \hat{h}_{ijk}(wait)dt + \int_{t_b}^t\hat{h}_{ijk}(transplant \, W_{m})dt))$$
...
This is the improvement in **median** quality-adjusted life-years from transplantation. Median years of life is equivalent to the point in time when the predicted survival function is 50%. Absolute survival benefit over a defined time period, the focus of this paper, is a **fundamentally different** measurement of the transplant efficiency that (if used to rank order candidates) would prioritize very different candidates.

![](long_term_projection.pdf)

## Methodologic issues with LFYT

1.  Projection

## Advantages of lives saved at 5-years over LYFT

    -   Improved methodology
        -   Joint semi-proportional hazards model instead of 6 seperate cox models
        -   Candidate characteristics interacted with time of transplant, donor characteristics
        -   Avoids extrapolation beyond the end of the data (informative censoring)
        -   No subjective quality-adjustment for dialysis time
    -   Ethical reasons
        -   LYFT is a pure life-years saved system, which is will save *fewer* lives at shorter time point
       
        -   LYFT severely disadvantages the elderly 
        -   Lives saved at 5-years better identifies the "most urgent" candidates per Final Rule

## Focusing on life-years shifts allocation to the young

By projecting out to median survival (far beyond the )

However this is just one relevant point in the distribution of survival times. Alternative metrics include

## LFYT models found similar distribution of short term-survival benefit

![](LFYT_paper.pdf)

The LYFT models find the same core result as we did- greater short-term survival benefit for the elderly. But the reason the younger recipients catch up is they need to extrapolate to the median survival time

-   Absolute improvement in survival probability at a specified time. This generates the **lives saved per kidney transplantated** at a certain time

$$Benefit =  S(5 \space years | transplant) - S(5 \space years| wait) dt $$

-   Restricted mean survival time (RSMT): area under the curve

$$Benefit = \int_{t_{transplant}}^{t_{end}} S(t | transplant) - S(t | wait) dt $$

This quantifies "lives saved over time"

```{r}
survival_benefit(t_transplant = 500, 782, 80, 2, 1, 0, 15, 25)
```
...
-   Add survival benefit to the graph

-   Fixed

    -   Cold ischemic time maybe non-linear
    -   fix days on waitlist
    -   add survival benefit in absolute improvement in days
    -   add % to each term in the model
    -   post-tranpslant survival by ischemic time quartile

-   Survival benefit by center

-   Additional analyses

    -   Age, racial, and gender equity
    -   Quantify magnitude of center effects

-   Remaining modeling decisions

    -   Adults, 2005-2010, kidney-alone: is this the right group?
